package com.abilix.explainer.balancecar;
...
public class BalanceCarData {
    ...
    private BalanceCarData() {
        if (GlobalConfig.STM_VISION_C >= STM_VERSION_NUTTX_BASE) {
            //nuttx版本平衡系数调整
            N = N * 0.82f;
    ...
    public static byte[] dealwheel_big_new(int value) {
        ...
        try {
           return SPUtils.request(motor_send);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
