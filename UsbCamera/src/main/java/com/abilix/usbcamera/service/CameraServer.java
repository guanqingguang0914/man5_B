...
import com.abilix.usbcamera.R;
import com.abilix.usbcamera.RobotCameraStateCode;
import com.abilix.usbcamera.client.TakePictureCallback;
import com.abilix.usbcamera.encoder.MediaAudioEncoder;
import com.abilix.usbcamera.encoder.MediaEncoder;
import com.abilix.usbcamera.encoder.MediaMuxerWrapper;
import com.abilix.usbcamera.encoder.MediaSurfaceEncoder;
import com.abilix.usbcamera.glutils.RendererHolder;
...
public final class CameraServer extends Handler {
    ...
    // jingh add
    private static IPreviewCallback mIPreviewCallback;
    private ITakePictureCallback mITakePictureCallback;

    public static int brightnessS = -1;
    ...
    public void release() {
        ...
        disconnect();
        mCallbacks.kill();
        mIPreviewCallback = null;
        mITakePictureCallback = null;
    ...
    public void resize(final int width, final int height) {
        LogMgr.d(String.format("resize(%d,%d)", width, height));
        if (!isRecording()) {
            mFrameWidth = width;
            mFrameHeight = height;
            if (mRendererHolder != null) {
    ...
    public void connect() {
        ...
        if (!thread.isCameraOpened()) {
            ...
            LogMgr.d("already connected, just call callback");
            processOnCameraStart();
        }
    }

    public void connectSlave() {
        LogMgr.d("connectSlave:");
        final CameraThread thread = mWeakThread.get();
        if (thread.isCameraOpened()) {
            processOnCameraStart();
        }
    ...
    public void disconnect() {
        ...
        //synchronized (thread.mSync) {
           // sendEmptyMessage(MSG_PREVIEW_STOP);
            ...
            // wait for actually preview stopped to avoid releasing
    ...
    public void captureStill(final String path, final TakePictureCallback takePictureCallback) {
        if (mRendererHolder != null) {
            mRendererHolder.captureStill(path,takePictureCallback);
            sendMessage(obtainMessage(MSG_CAPTURE_STILL, path));
/*            // jingh add 拍照成功回调
            try {
                if (mITakePictureCallback == null) {
                    LogMgr.e("captureStill===>mITakePictureCallback is null");
                }
                if (mITakePictureCallback != null) {
                    LogMgr.e("captureStill===>拍照成功");
                    mITakePictureCallback.onState(RobotCameraStateCode.SAVE_PICTURE_SUCESS);
                }
    ...
    public void takePicture(String path, ITakePictureCallback callback) {
        LogMgr.e("CameraServer===>拍照路径：" + path);
       mITakePictureCallback = callback;
        captureStill(path,mTakePictureCallback);
    }
    private TakePictureCallback mTakePictureCallback=new TakePictureCallback() {
        ...
        public void onState(int state,byte[] data) {
            ...
            if (mITakePictureCallback != null) {
                try {
                    LogMgr.e("CameraServer===>拍照大小：" + data.length);
                    mITakePictureCallback.onState(state,data);
    ...
    public void setBrightnessS(int brightness) {
        if (!isRecording()) {
            sendMessage(obtainMessage(MSG_SETBRIGHTNESSS, brightness));
        }
    }
    ...
    public int getBrightnessS() {
        final CameraThread thread = mWeakThread.get();
        if (thread != null) {
            brightnessS = thread.handleGetBrightnessS();
        }
        return brightnessS;
    }

    public void preview(IPreviewCallback callback) {
        LogMgr.e("preview===>");
        mIPreviewCallback = callback;
    }

    // ********************************************************************************
    private void processOnCameraStart() {
        LogMgr.d("processOnCameraStart:");
        try {
            ...
            for (int i = 0; i < n; i++) {
                if (!((CallbackCookie) mCallbacks.getBroadcastCookie(i)).isConnected)
                    try {
                        ...
                        LogMgr.e("failed to call IOverlayCallback#onFrameAvailable");
                    }
            }
        ...
        for (int i = 0; i < n; i++) {
            if (((CallbackCookie) mCallbacks.getBroadcastCookie(i)).isConnected)
                try {
                    ...
                    LogMgr.e("failed to call IOverlayCallback#onDisConnected");
                }
        }
        mCallbacks.finishBroadcast();
    }
    ...
    private static final int MSG_PREVIEW = 10;
    private static final int MSG_SETBRIGHTNESSS = 11;
    private static final int MSG_GETBRIGHTNESTT = 12;
    ...
    public void handleMessage(final Message msg) {
        ...
        switch (msg.what) {
            ...
            case MSG_RELEASE:
                ...
                break;
            case MSG_SETBRIGHTNESSS:
                thread.handleSetBrightnessS((Integer) (msg.obj));
                break;
            case MSG_GETBRIGHTNESTT:
                brightnessS = thread.handleGetBrightnessS();
                break;
    ...
    private static final class CameraThread extends Thread {
        ...
        private CameraThread(final Context context, final UsbControlBlock ctrlBlock) {
            super("CameraThread");
            LogMgr.d("CameraThread Constructor:");
        ...
        public void handleOpen() {
            ...
            //handleClose();
            synchronized (mSync) {
                LogMgr.e("初始化 UVCCamera");
                mUVCCamera = new UVCCamera();
                mUVCCamera.open(mCtrlBlock);
                LogMgr.i("supportedSize:" + mUVCCamera.getSupportedSize());
            }
            mHandler.processOnCameraStart();
        }

        public void handleClose() {
            LogMgr.d("handleClose:");
            //handleStopRecording();
        ...
        public void handleCaptureStill(final String path) {
            LogMgr.d("handleCaptureStill:");

            mSoundPool.play(mSoundId, 0.2f, 0.2f, 0, 0, 1.0f); // play shutter
            // sound
        }


        public void handleStartRecording() {
            LogMgr.d("handleStartRecording:");
            try {
                if ((mUVCCamera == null) || (mMuxer != null))
                    return;
                mMuxer = new MediaMuxerWrapper(".mp4"); // if you record audio
                // only, ".m4a" is also
                // OK.
                ...
                LogMgr.e("startCapture:" + e);
            }
        }

        public void handleStopRecording() {
            LogMgr.d("handleStopRecording:mMuxer=" + mMuxer);
            if (mMuxer != null) {
        ...
        public void handleUpdateMedia(final String path) {
            LogMgr.d("handleUpdateMedia:path=" + path);
            final Context context = mWeakContext.get();
            if (context != null) {
                try {
                    LogMgr.i("MediaScannerConnection#scanFile");
                    MediaScannerConnection.scanFile(context, new String[]{path}, null, null);
                } catch (final Exception e) {
                    LogMgr.e("handleUpdateMedia:" + e);
                }
            } else {
                LogMgr.w("CaptureActivity already destroyed");
                // give up to add this movice to MediaStore now.
                // Seeing this movie on Gallery app etc. will take a lot of
                // time.
                handleRelease();
            }
        }
        ...
        // if you need frame data as ByteBuffer on Java side, you can use this
        // callback method with UVCCamera#setFrameCallback
            ...
            public void onFrame(final ByteBuffer frame) {
                ...
                try {
                    if (mIPreviewCallback != null) {
                        LogMgr.e("回调预览数据");
                        mIPreviewCallback.onPreviewFrame(data);
                    }
                } catch (RemoteException e) {
                    e.printStackTrace();
                }
        ...
        private final MediaEncoder.MediaEncoderListener mMediaEncoderListener = new MediaEncoder.MediaEncoderListener() {
            ...
            public void onPrepared(final MediaEncoder encoder) {
                LogMgr.d("onPrepared:encoder=" + encoder);
                mIsRecording = true;
                if (encoder instanceof MediaSurfaceEncoder)
                    try {
                        ...
                        LogMgr.e("onPrepared:" + e);
                    }
            }

            @Override
            public void onStopped(final MediaEncoder encoder) {
                LogMgr.v("onStopped:encoder=" + encoder);
                if ((encoder instanceof MediaSurfaceEncoder))
                    try {
                        mIsRecording = false;
        ...
        private void loadSutterSound(final Context context) {
            ...
            try {
                ...
                streamType = AudioManager.STREAM_SYSTEM; // set appropriate
                // according to your
                // app policy
            }
            if (mSoundPool != null) {
        ...
        public void handleSetBrightnessS(int brightness) {
            LogMgr.d("handleSetBrightnessS:");
            synchronized (mSync) {
                if (mUVCCamera == null) mUVCCamera = new UVCCamera();
                mUVCCamera.updateCameraParams();
                mUVCCamera.setBrightnessS(brightness);
            }
        }

        public int handleGetBrightnessS() {
            int result = -1;
            LogMgr.d("handleGetBrightnessS:");
            synchronized (mSync) {
                if (mUVCCamera == null) mUVCCamera = new UVCCamera();
                result = mUVCCamera.getBrightnessS();
            }
            return result;
        }
